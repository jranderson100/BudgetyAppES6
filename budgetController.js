...
export const addItem = (type, des, val) => {
  ...
  if (type === 'exp') {
    ...
    if(data.totals.inc > 0) {
      newItem.percentage = Math.round((newItem.value / data.totals.inc) * 100);
    }  else {
      this.percentage = -1;
}

console.log(newItem.percentage + 'this is the newItems percentage');
} else if (type === 'inc') {
...
console.log("I've calculated percentages");
console.log(data.percentage + '' + data.budget)
});
};
...
export const getAllPercentages = () => {
  ...
  return allPerc;
  console.log(allPerc + "hellothereI'm printing allPerc");
  };
